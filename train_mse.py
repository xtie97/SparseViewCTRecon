...
def train(param):
    ...
    train_dataset = DataGenerator(data_dir, df_train, transform=True) 
    ...
    val_dataset = DataGenerator(data_dir, df_val, transform=False)
    ...
    for epochID in range(param["nepoch"]): 
        ...
        losstrain_list.append(loss_gen)
        lossval_list.append(loss_gen_val)
        epochID_list.append(epochID)
        if param["save_all"] == 1:
            model_name_ = model_name.replace(".pth.tar", "_{:03d}.pth.tar".format(epochID))
            torch.save({'epoch': epochID, 'state_dict': model.state_dict()}, model_name_)
...
def epochTrain (model, dataLoader, optimizer_G, loss_l2): 
    ...
    for inp, target in tqdm (dataLoader):
        ...
        lossvalue = loss_l2(output, target_var)
    ...
    loss_gen = loss_gen.cpu().detach().numpy() / loss_gen_Norm
