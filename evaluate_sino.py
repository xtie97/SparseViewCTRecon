...
def test(param):
    ...
    if os.path.isfile(model_name):
        ...
        raise Exception("=> Fail to find the checkpoint")
    val_dataset = DataGenerator_sino(data_dir, df_test, param['potential'], param['num_views'])
...
def epochVal(model, dataLoader, filenames, results_sep_folder): 
    ...
    with torch.no_grad():
        for inp, target in tqdm (dataLoader):
            ...
            output = output * mask + target_var * (1-mask)  
            
            output = output.cpu().detach().numpy().squeeze() * weight_factor
